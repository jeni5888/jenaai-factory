...
export class FlowctlNotFoundError extends Error {
  ...
  constructor(startDir: string, searchedPaths: string[]) {
    ...
    const msg = `flowctl not found. Run \`/jenaai-flow:setup\` or ensure jenaai-flow plugin is installed. Searched: ${pathList}`;
    super(msg);
...
export async function getFlowctlPath(startDir?: string): Promise<string> {
  ...
  // 2a. ./plugins/jenaai-flow/scripts/flowctl (from cwd)
  const jenaaiPluginPath = `${cwd}/plugins/jenaai-flow/scripts/flowctl`;
  searchedPaths.push(jenaaiPluginPath);
  result = await tryFlowctl(jenaaiPluginPath);
  if (result) {
    cache = result;
    return result.path;
  }

  // 2b. ./plugins/flow-next/scripts/flowctl (from cwd)
  ...
  if (repoRoot && repoRoot !== cwd) {
    ...
    const repoJenaaiPluginPath = `${repoRoot}/plugins/jenaai-flow/scripts/flowctl`;
    searchedPaths.push(repoJenaaiPluginPath);
    result = await tryFlowctl(repoJenaaiPluginPath);
    if (result) {
      cache = result;
      return result.path;
    }
